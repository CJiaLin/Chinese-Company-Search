...
def convert_to_dict_pre(sentence, pred):
    ...
    while i < len(pred):
        ...
        if pred[i][0] == "B":
            ...
            s = sentence[i]
            start = j
            j +=  1 if sentence[i] == '[UNK]' else len(sentence[i].strip('#'))
            i += 1
            while i < len(pred) and pred[i].startswith('I-'): #  == "I-" + type:
                s += sentence[i].strip('#')
                j +=  1 if sentence[i] == '[UNK]' else len(sentence[i].strip('#'))
                i += 1
            # end = i
            end = j
            j -=  1 if sentence[i-1] == '[UNK]' else len(sentence[i-1].strip('#'))
            i -= 1
            ...
            if type not in dic:
                dic[type] = []
            dic[type].append([sentence[i], j,  j + 1 if sentence[i] == '[UNK]' else len(sentence[i].strip('#'))])
