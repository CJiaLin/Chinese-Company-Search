...
#         print(batch_data, batch_token_starts, batch_labels)
        # print(batch_data.shape, batch_token_starts.shape, batch_labels.shape)
        # print(batch_labels.min(), batch_data.min(), batch_token_starts.min())
...
#             continue
        ...
        improve_loss = min_val_loss - val_metrics['loss']
        if improve_f1 > 1e-5:
            best_val_f1 = val_f1
            model_dir_new = config.model_dir + 'best_model/'
#             model_dir_new = config.model_dir + str(val_f1)[:6] +'_' + str(val_metrics['loss'])[:6] +'_' + str(epoch) + '/'
            ...
            torch.save(model, model_dir_new + 'model.bin')
#             model.save_pretrained(model_dir_new)
        ...
        if improve_loss > 1e-5:
            min_val_loss = val_metrics['loss']
            model_dir_new = config.model_dir + 'best_loss_model/'
            if not os.path.exists(model_dir_new):               #判断文件夹是否存在
                os.makedirs(model_dir_new)                       #新建文件夹
            torch.save(model, model_dir_new + 'model.bin')
#             model.save_pretrained(model_dir_new)
            logging.info("--------Save best loss model!--------")
